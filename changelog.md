...
1. **全新UI，白纸一张，自由发挥。**（后台外观设置——主题色调自定义搭配，填写white-white-white 即可）
2. 全新的tab、搜索界面、link友情链接页面界面
3. 优化表格的样式
3. **全新的图标库，更轻，更现代化，现可以在侧边栏导航中使用**（可以查看使用文档——自定义栏目中有说明）
2. **图片头图，更节省空间，更大的视觉冲击力**（在文章编辑页面——头图样式——选择图片头图即可）
3. 左侧边栏头像两种样式切换（竖排/横排，在外观设置——外观设置开关——左侧边栏竖排头像样式 开关切换）
4. 增加头图图片版权提示（@视觉中国）（在文章编辑页面——头图版权说明 填写设置项即可，在文章页面的头图右下角会有版权提示）
5. 文章标签来了！在文章页面会看到当前文章页面的标签
6. 增加了主题接口的安全性，避免恶意调用
7. **增加了自适应的相册布局**（使用文档——增强功能——插入相册可以看到说明用法）
8. 修复加密分类会输出description的问题
9. **优化前端搜索速度，搜索支持空格分割多个关键词合并搜索结果**
10. 优化Firefox下的滚动性能
11. 修复平滑滚动js在博客为加载完成前的滚动问题，并只在Windows上启动
12. 修复表情代码当不存在表情文件时候，显示文字
13. 图片下方增加文字描述，显示alt标签的文字
14. 增加时光机上传图片无损压缩
15. **主题的云存储设置现在支持腾讯云！**（外观设置——速度优化——云存储空间设置中可以配置）
16. 优化了小头图在某些分辨率下的样式问题
17. 更新fancybox版本信息
18. 修复灯箱和按钮点击后滚动条偏移问题
19. 现在，打赏页面支付二维码不填写则不显示
20. 增加一键关闭评论系统的功能（评论设置——评论系统设置——选择关闭评论系统）
21. 修复置顶导航模式下的底部菜单遮挡问题
22. **更新了编辑器的图标，已经增加编辑器快捷插入增强功能的按钮，编辑器支持对文字、颜色进行调整**
23. 增加了对右侧边栏进行设置是否显示的开关（页面元素显示设置——右侧边栏元素控制）
24. 文章中的图片除了表情自动居中
25. 增加了云存储处理文章图片自定义参数后缀
26. **增加、编辑文章时候的搜索构建算法优化，速度更快**（对于文章数目多的可能很明显）
27. 修复handsome插件构建搜索索引会包含加密分类文章的问题
27. 修复微信时光机在某些情况下不可用的问题
28. 优化了「取消盒子模式」下的样式（标题居中，中间文章距离两边会有距离）
29. 修复友情链接页面如果图片挂了的样式问题
30. 增加了首页列表最前方广告位（外观设置——开发者设置中可以看到）
31. 增加了文章页面的分享开关（在外观设置——主题增强功能里面可以看到设置）
32. 修复折叠模式、置顶模式下的分类展开问题
33. 对于文章中的内链、外链进行样式区分，同时内链直接无刷新打开，外链新窗口打开
34. 微信时光机支持发送私密说说（\#内容）
35. **增加了首页轮播图展示**（外观设置——高级设置 里面有设置项）
36. 支持php7.4
